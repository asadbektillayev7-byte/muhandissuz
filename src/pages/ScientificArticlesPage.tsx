...
const ScientificArticlesPage = () => {
  ...
  const { data: articles, isLoading } = useQuery({
    queryKey: ["public-scientific-articles"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("scientific_articles")
        .select("*, categories(name, name_en, color)")
        .eq("status", "published")
        ...
        <section className="container mx-auto px-4 py-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered?.map((article: any) => (
              <Link key={article.id} to={article.pdf_url || "#"}>
                <article className="group bg-card rounded-xl overflow-hidden border border-border hover:shadow-lg transition-shadow cursor-pointer">
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-sm text-muted-foreground line-clamp-2">
                      {lang === "en" && article.abstract_en ? article.abstract_en : article.abstract}
                    </p>
                  </div>
                </article>
              </Link>
