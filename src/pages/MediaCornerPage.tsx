  ...
  const { data: items, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [albums, videos] = await Promise.all([
        supabase.from("oav_albums").select("*").eq("is_active", true).order("sort_order", { ascending: true }),
        supabase.from("oav_videos").select("*").eq("is_active", true).order("sort_order", { ascending: true }),
      ]);
      if (albums.error) throw albums.error;
      if (videos.error) throw videos.error;
      return [
        ...(albums.data || []).map((a: any) => ({ ...a, type: "photo", thumbnail_url: a.cover_url, url: null, event_date: a.album_date })),
        ...(videos.data || []).map((v: any) => ({ ...v, type: "video", url: v.video_url })),
      ];
    },
  });
